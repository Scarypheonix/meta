...
func TestSemicolonInsertion(t *testing.T) {
	...
	cases := []struct {
		...
		{"rule 3: never before a leading dot, so a method chain may wrap",
			"a\n.b()", []Kind{Ident, Dot, Ident, LParen, RParen}},
		{"a dot on the same line is unaffected",
			"a.b\nc", []Kind{Ident, Dot, Ident, Semi, Ident}},

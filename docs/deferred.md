...
| Native heap collection | still needs, on top of the above: the stack-map table itself, emitted per call site into read-only data and reachable from the runtime block; and the collector that walks it. Scoped to a single-space, stop-the-world collector to start — no write barrier, no card table, no generational split (ADR-0021's "Consequences") — with going generational left for a later phase | Phase 5 |

...
| A `-O2` fixed-point non-convergence inside a `while` loop that assigns two loop-carried variables from the loop variable two different ways each iteration (minimally: `a = i; b = i + 1; i = i + 1;` inside `while i < N`, no struct or allocation needed) | found while building the native collector's own stress test (`docs/spec/10-examples.md` #13, which sidesteps it with `Pair { a: i, b: i }` instead of `Pair { a: i, b: i + 1 }`); some pair of passes in `internal/opt`'s pipeline (`runPasses`, `opt.go`) undoes the other's work indefinitely, past `maxRounds`, on this shape. Confirmed unrelated to the collector or to structs at all -- not yet root-caused | Phase 4 |

...
| A value's static kind, for a field/payload/tuple-element read, a call's result, a parameter and a capture | delivered in Phase 5 (ADR-0021): `bytecode.Instr.Kind` and `bytecode.Fn.ParamKinds`/`CaptureKinds`, populated by `internal/compile` from the checker's own types, are the seed data a stack map needs and the IR cannot recover on its own once built from untyped bytecode. Nothing consumes it yet (`internal/compile/kind_test.go` verifies the data directly) | done |
| Native heap collection | still needs, on top of the kind data above: an `internal/backend`-only pass propagating a kind to every remaining IR value structurally (mirroring `closures.go`'s `resolveClosureCalls`); the register allocator placing every reference-kind spill slot in one contiguous run below the raw ones (already specified, spec/11-codegen.md's "Stack frames"); the stack-map table itself, emitted per call site into read-only data and reachable from the runtime block; and the collector that walks it. Scoped to a single-space, stop-the-world collector to start — no write barrier, no card table, no generational split (ADR-0021's "Consequences") — with going generational left for a later phase | Phase 5 |

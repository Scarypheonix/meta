...
// There is no self-source row here any more. It asked whether the two compilers agree on
// stage1's own instantiation set; `TestStage1CompilesItself` asks whether they agree on the
// *bytes of the executable* built from that same source, which is downstream of the whole
// instantiation set and of everything after it. A test whose conclusion another test already
// establishes more strongly is suite time spent twice.
